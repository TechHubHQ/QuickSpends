...
export default function QSAccountDetailsScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (currentAccount) {
        ...
        const accountTransactions = allRecent.filter(
          (t) => t.account_id === id,
        );
        ...
        const monthIn = accountTransactions
          .filter((t) => {
            ...
            return t.type === "income" && new Date(t.date) >= startOfMonth && !isReconciliation;
          })
        ...
        const monthOut = accountTransactions
          .filter((t) => {
            ...
            return t.type === "expense" && new Date(t.date) >= startOfMonth && !isReconciliation;
          })
          .reduce((sum, t) => sum + t.amount, 0);
  ...
  const displayedTransactions = expanded
    ...
    : allFilteredTransactions.slice(0, 5);
    ...
    <View style={styles.container}>
      ...
      >
        ...
        >
          <QSAccountCard
            account={account}
            monthIn={stats.monthIn}
            monthOut={stats.monthOut}
          />
          ...
          {account.type === "cash" ? (
            ...
            <>
              ...
              >
                ...
                >
                  <MaterialCommunityIcons
                    name={activeFilter === "all" ? "filter-variant" : "filter"}
                    size={24}
                    color={
                      activeFilter !== "all"
          ...
          <View style={styles.transactionList}>
            {displayedTransactions.length > 0 ? (
              <>
                ...
                {allFilteredTransactions.length > 5 && (
                  <TouchableOpacity
      ...
      >
        <View style={{ paddingBottom: 30 }}>
          ...
          >
            <Text
              style={{ color: theme.colors.text, fontSize: 18, marginRight: 8 }}
            >
              ₹
            </Text>
            <TextInput
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              ...
              try {
                ...
                // Search for Reconciliation first, then fallback to Adjustment
                let category = categories.find((c) => c.name === "Reconciliation") || categories.find((c) => c.name === "Adjustment");

                if (!category) {
                  // Create Reconciliation category if it doesn't exist
                ...
                if (transactionId) {
                  ...
                  Toast.show({
                    ...
                    text1: "Reconciled",
                    text2:
                      "Wallet balance updated with a correction transaction.",
                  });
                } else {
                  Toast.show({
                    type: "error",
