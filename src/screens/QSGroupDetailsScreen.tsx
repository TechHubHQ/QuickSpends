...
export default function QSGroupDetailsScreen() {
  ...
  const { user } = useAuth();
  const { getGroupDetails, addMembersToGroup, acceptGroupInvite, deleteGroup } =
    useGroups();
  ...
  const handleMembersAdd = async (
    newMembers: { name: string; email: string; id?: string }[],
  ) => {
  ...
  const chartPath = useMemo(() => {
    ...
    if (sorted.length === 0) {
      ...
      points = [
        { x: 0, y: sorted[0].amount },
        { x: 300, y: sorted[0].amount },
      ];
    } else {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return result;
  }, [group, filterUserId, filterType, sortOption]);
  ...
  const memberStats = useMemo(() => {
    ...
    // Create a map of transaction IDs that are currently visible
    const visibleTxIds = new Set(filteredTransactions.map(t => t.id));

    // DEBUG: Check splits availability
    console.log('[QSGroupDetails] MemberStats Debug:', {
      totalSplits: group.splits?.length,
      visibleTxCount: visibleTxIds.size,
      sampleSplit: group.splits?.[0]
    });

    return group.members.map(member => {
      // Calculate dynamic share for this member based on visible transactions
      const dynamicShare = (group.splits || [])
        .filter(s => s.user_id === member.id && visibleTxIds.has(s.transaction_id))
        .reduce((sum, s) => sum + s.amount, 0);

      // Calculate dynamic paid for this member (though we only show share for now)
      const dynamicPaid = filteredTransactions
  ...
  let badgeBg = theme.isDark
    ? "rgba(239, 68, 68, 0.1)"
    : "rgba(239, 68, 68, 0.1)";
  let badgeBorder = theme.isDark
    ? "rgba(239, 68, 68, 0.2)"
    : "rgba(239, 68, 68, 0.2)";
  ...
  const balanceText = isSettled
    ? "Settled up"
    : isOwed
      ? `You are owed ₹${Math.abs(stats.myBalance).toFixed(0)}`
      : `You owe ₹${Math.abs(stats.myBalance).toFixed(0)}`;
      ...
      >
              ...
              <View style={{ flexDirection: "row", gap: 12, marginTop: 4 }}>
                ...
                >
                  <MaterialIcons
                    ...
                    color="#fca5a5"
                  />
                  <Text style={[styles.balanceText, { color: "#fca5a5" }]}>
                    {formatCurrency(stats.totalGroupSpends)}
                  </Text>
                </View>
                ...
                >
                  <MaterialIcons name="arrow-upward" size={14} color="#86efac" />
                  <Text style={[styles.balanceText, { color: "#86efac" }]}>
                    {formatCurrency(stats.totalGroupIncome)}
                  </Text>
                </View>
              </View>
            </View>
        ...
        >
          ...
          <ActionButton
            ...
            label="Split"
            onPress={() =>
              router.push({ pathname: `/split-expense/${group.id}` } as any)
            }
          />
          <ActionButton
            icon="payments"
            label="Settle Up"
            onPress={() =>
              router.push({ pathname: `/settle-up/${group.id}` } as any)
            }
          />
          <ActionButton
            icon="person-add"
          ...
          >
            <View>
              ...
              <Text style={{ fontSize: 12, color: theme.colors.textSecondary }}>
                Join to participate in expenses
              </Text>
            </View>
            <TouchableOpacity
              style={{
            ...
            {memberStats.map((member, index) => (
              ...
              >
                ...
                >
                  {member.status === "invited"
                    ? "Invited"
                    : `Share ₹${member.dynamicPaid.toFixed(0)}`}
                </Text>
              </Animated.View>
            ))}
          </ScrollView>
          ...
          {(filterUserId !== null || filterType !== "all" || sortOption !== "date_desc") && (
            ...
            >
              <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                ...
                >
                  {filterUserId
                    ? group.members.find((m) => m.id === filterUserId)?.username +
                    "'s spend"
                    : "Filtered results"}
                  ...
                  >
                    <Text
                      style={[
                        styles.filterText,
                        {
                          color: filterType === type ? "#FFF" : theme.colors.text,
                        },
                      ]}
                    >
                ...
                >
                  <Text
                    style={[
                      ...
                      {
                        color:
                          filterUserId === null ? "#FFF" : theme.colors.text,
                      },
                    ]}
                  >
                    All Users
                  ...
                  >
                    <Text
                      style={[
                        ...
                        {
                          color:
                            filterUserId === m.id ? "#FFF" : theme.colors.text,
                        },
                      ]}
                    >
                  ...
                  >
                    <TouchableOpacity
                      style={[
                        ...
                        {
                          borderBottomWidth:
                            i === displayedTransactions.length - 1 ? 0 : 1,
                        },
                      ]}
                      onPress={() =>
                        router.push({
                    ...
                    >
                      <View
                        style={[
                          ...
                          {
                            backgroundColor:
                              (t.category_color || theme.colors.primary) + "20",
                          },
                        ]}
                      >
                        <MaterialCommunityIcons
                      ...
                      <View style={styles.activityContent}>
                        ...
                        <View style={styles.activityMeta}>
                          <Text style={styles.activitySubtitle}>
                            {t.user_id === user?.id ? "You" : t.payer_name} paid
                            • {t.category_name}
                          </Text>
                        </View>
                      </View>
