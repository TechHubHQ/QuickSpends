...
export default function QSTransactionDetailsScreen() {
    ...
    const { date, time } = transaction ? formatDate(transaction.date) : { date: 'Oct 24, 2023', time: '4:30 PM' };
    ...
    const data = transaction || {
        ...
        category_icon: "cart",
        type: "expense",
        description: "Weekly grocery run for the party. Bought extra snacks and drinks.",
        ...
        <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
            ...
            <ScrollView contentContainerStyle={styles.content}>
                ...
                <Animated.View entering={FadeInDown.delay(100).springify()} style={styles.heroSection}>
                    ...
                    <Text style={[styles.heroMerchant, { color: theme.colors.textSecondary }]}>
                        {data.name}
                    </Text>
                </Animated.View>
                ...
                <Animated.View entering={FadeInUp.delay(200).springify()} style={[
                    ...
                    <Text style={[styles.cardTitle, { color: theme.colors.text }]}>GENERAL INFO</Text>

                    <View style={[styles.row, { borderBottomColor: theme.colors.border }]}>
                        <View style={styles.rowLeft}>
                            <View style={[styles.iconBox, { backgroundColor: isDark ? 'rgba(59, 130, 246, 0.2)' : 'rgba(59, 130, 246, 0.1)' }]}>
                                <MaterialCommunityIcons name="calendar-blank" size={16} color="#3B82F6" />
                            </View>
                            <Text style={[styles.rowLabel, { color: theme.colors.textSecondary }]}>Date</Text>
                        </View>
                        <Text style={[styles.rowValue, { color: theme.colors.text }]}>{date}</Text>
                    </View>

                    <View style={[styles.row, { borderBottomColor: theme.colors.border }]}>
                        <View style={styles.rowLeft}>
                            <View style={[styles.iconBox, { backgroundColor: isDark ? 'rgba(168, 85, 247, 0.2)' : 'rgba(168, 85, 247, 0.1)' }]}>
                        ...
                        <Text style={[styles.rowValue, { color: theme.colors.text }]}>{time}</Text>
                    </View>


                    {/* Description in General Info */}
                    {!!data.description && (
                        <View style={[styles.row, { borderBottomWidth: 0, flexDirection: 'column', alignItems: 'flex-start', gap: 8, paddingTop: 12, borderTopWidth: 1, borderTopColor: theme.colors.border }]}>
                            <Text style={[styles.rowLabel, { color: theme.colors.textSecondary }]}>Note</Text>
                            <Text style={[styles.notesText, { color: theme.colors.text, fontSize: 14 }]}>
                                {data.description}
                            </Text>
                        </View>
                    )}
                ...
                <Animated.View entering={FadeInUp.delay(300).springify()} style={[
                    ...
                    <View style={[styles.row, { borderBottomColor: theme.colors.border }]}>
                        <Text style={[styles.rowLabel, { color: theme.colors.textSecondary }]}>Category</Text>
                        <View style={styles.categoryBadge}>
                            <View style={[styles.categoryDot, { backgroundColor: data.category_color || theme.colors.text }]} />
                            <Text style={[styles.rowValue, { color: theme.colors.text }]}>{data.category_name || 'Uncategorized'}</Text>
                        </View>
                    </View>

                    <View style={[styles.row, { borderBottomColor: theme.colors.border }]}>
                        <Text style={[styles.rowLabel, { color: theme.colors.textSecondary }]}>Account</Text>
                        <View style={styles.accountBadge}>
                            {/* Placeholder for Account Icon */}
                            <View style={[styles.accountIcon, { backgroundColor: theme.colors.backgroundSecondary }]} />
                            <Text style={[styles.rowValue, { color: theme.colors.text }]}>{data.account_name || 'Account'}</Text>
                        </View>
                    </View>

                    <View style={[styles.row, { borderBottomWidth: data.description ? 1 : 0, borderBottomColor: theme.colors.border }]}>
                        <Text style={[styles.rowLabel, { color: theme.colors.textSecondary }]}>Type</Text>
                        <Text style={[styles.rowValue, { color: theme.colors.text, textTransform: 'capitalize' }]}>{data.type}</Text>
                    </View>
                ...
                {!!(data.trip_id || data.group_id || data.recurring_id || data.savings_id) && (
                    <Animated.View entering={FadeInUp.delay(400).springify()} style={[
                            ...
                            <TouchableOpacity style={[
                                ...
                                <View style={styles.tagLeft}>
                                    <View style={[styles.iconBox, { backgroundColor: isDark ? 'rgba(236, 72, 153, 0.2)' : 'rgba(236, 72, 153, 0.1)' }]}>
                                        <MaterialCommunityIcons name="airplane" size={16} color="#EC4899" />
                                    </View>
                                    <View>
                                        <Text style={[styles.tagName, { color: theme.colors.text }]}>{data.trip_name || 'Trip'}</Text>
                                        <Text style={[styles.tagType, { color: theme.colors.textSecondary }]}>Trip</Text>
                                    </View>
                                </View>
                        ...
                        {/* Recurring Tag */}
                        {!!data.recurring_id && (
                            <TouchableOpacity style={[styles.tagRow, { borderBottomWidth: 0 }]}>
                                <View style={styles.tagLeft}>
                                    ...
                                    <View>
                                        <Text style={[styles.tagName, { color: theme.colors.text, textTransform: 'capitalize' }]}>{data.recurring_frequency || 'Monthly'}</Text>
                                        <Text style={[styles.tagType, { color: theme.colors.textSecondary }]}>Recurring</Text>
                                    </View>
                                </View>
                                <MaterialCommunityIcons name="chevron-right" size={20} color={theme.colors.textTertiary} />
                            </TouchableOpacity>
