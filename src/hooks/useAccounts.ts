...
export const useAccounts = () => {
  ...
  const createAccount = useCallback(
    async (account: Omit<Account, "id" | "is_active">) => {
      setLoading(true);
      setError(null);
      try {
        // 1. Create Account
        ...
        if (account.balance > 0) {
          ...
          const { data: existingCategory, error: catFetchError } =
            await supabase
              .from("categories")
              .select("id")
  ...
  const getAccountsByUser = useCallback(async (userId: string) => {
    ...
    try {
      ...
      // 2. Update balances
      const processedAccounts = processAccounts(accounts);
  ...
  const updateAccount = useCallback(
    async (
      id: string,
      updates: Partial<
        Omit<Account, "id" | "user_id" | "currency" | "is_active">
      >,
    ) => {
      setLoading(true);
      setError(null);
