...
export const useAnalytics = () => {
  ...
  const getNetWorth = useCallback(
    async (userId: string, options?: DateRangeOptions) => {
      ...
      try {
        ...
        const currentRealTimeNetWorth =
          currentTotalAssets - currentTotalLiabilities;
        ...
        if (end < now && !isSameDay(end, now)) {
          ...
          const { data: reductionTxns } = await supabase
            .from("transactions")
            .select(
              "amount, type, name, category:categories!transactions_category_id_fkey(name, parent:parent_id(name))",
            )
            .eq("user_id", userId)
            .gt("date", end.toISOString())
            .lte("date", now.toISOString()); // up to now

          const reductionChange = (reductionTxns || []).reduce(
            (sum, t: any) => {
        ...
        const sortedTxns = (rangeTransactions || []).sort(
        ...
        while (curr > start && iterations < MAX_ITERATIONS) {
          ...
          historyPoints.push({
            ...
            label: format(
              effectiveDate,
              intervalType === "monthly" ? "MMM" : "d MMM",
            ),
          });
        ...
        const startValue =
          finalHistory.length > 0 ? finalHistory[0].value : endNetWorth;
  ...
  const getSpendingInsights = useCallback(
    async (userId: string, options?: DateRangeOptions) => {
      ...
      try {
        const {
          startISO: currentPeriodStart,
          endISO: currentPeriodEnd,
          start: startObj,
          end: endObj,
        } = getRangeBounds(options);

        // Calculate actual days in period for average calculation
        const daysInPeriod = Math.max(
          1,
          Math.ceil(
            (endObj.getTime() - startObj.getTime() + 1) / (1000 * 60 * 60 * 24),
          ),
        );

        const previousPeriodEndObj = new Date(startObj.getTime());
        const previousPeriodStartObj = new Date(
          previousPeriodEndObj.getTime() -
        ...
        const { data: currentTransactions } = await supabase
          .from("transactions")
          .select(
            "amount, name, category:categories (id, name, parent:parent_id(name))",
          )
          .eq("user_id", userId)
          .eq("type", "expense")
        ...
        const topCat = Array.from(catMap.values()).sort(
          (a, b) => b.total - a.total,
        )[0];
  ...
  const getMerchantSpending = useCallback(
    async (userId: string, options?: DateRangeOptions) => {
      setLoading(true);
      try {
        const { startISO, endISO } = getRangeBounds(options);
  ...
  const getSpendingVelocity = useCallback(async (userId: string) => {
    ...
    try {
      ...
      const currentTotal = (currentMonthTxns || []).reduce(
        (sum, t) => sum + t.amount,
        0,
      );
      ...
      const avgHistorical =
        Object.values(monthlyBuckets).length > 0
          ? Object.values(monthlyBuckets).reduce((a, b) => a + b, 0) /
            Object.values(monthlyBuckets).length
          : currentTotal;
      ...
      return {
        ...
        projectedOverspend:
          status === "ahead"
            ? (currentTotal / dayOfMonth) * totalDaysInMonth - avgHistorical
            : 0,
        dailyVelocity,
      } as SpendingVelocity;
    } catch (err: any) {
    ...
    try {
      ...
      (loans || []).forEach((l) => {
        ...
        if (
          !nextPaymentDate ||
          (l.next_due_date &&
            new Date(l.next_due_date) < new Date(nextPaymentDate))
        ) {
      ...
      return {
        ...
        progress:
          totalDebt > 0 ? ((totalDebt - remainingDebt) / totalDebt) * 100 : 0,
        interestPaid: 0,
        nextPaymentDate,
      } as DebtHealth;
    } catch (err: any) {
