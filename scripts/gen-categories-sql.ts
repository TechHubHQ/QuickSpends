...
async function generate() {
  ...
  for (const cat of DEFAULT_CATEGORIES) {
    ...
    sql += `    ON CONFLICT ON CONSTRAINT unique_default_category_name_type_parent DO UPDATE SET icon = EXCLUDED.icon, color = EXCLUDED.color\n`;
    ...
    if (cat.subCategories) {
      for (const sub of cat.subCategories) {
        ...
        sql += `    ON CONFLICT ON CONSTRAINT unique_default_category_name_type_parent DO NOTHING;\n`;
      }
      sql += `\n`;
    }
  }
